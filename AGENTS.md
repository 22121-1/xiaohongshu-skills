...
- `origin`：个人 fork，`https://github.com/adyingfish/xiaohongshu-skills.git`。个人分支推送到这里。
- `upstream`：原仓库，`https://github.com/autoclaw-cc/xiaohongshu-skills.git`。从这里获取更新，向这里提交通用功能和修复的 PR（拉取请求）。
...
`personal` 初始版本基于已验证的 `feat/direct-messages`。这只是自用分支的起点，不代表私信功能已经被原仓库合并。不要自动整合其他待审查的 PR。
...
日常安装使用的检出目录固定在 `personal`；需要同时开发其他功能时，使用单独工作树，避免切换分支改变正在使用的技能版本。
...
gh pr create --repo adyingfish/xiaohongshu-skills \
