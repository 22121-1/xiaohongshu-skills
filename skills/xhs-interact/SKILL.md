...
读取会话或通知可能由网页自然更新已读状态。所有列表按返回的完整性字段说明范围；详细参数与限制见 [私信、通知与关注管理](../../docs/interact-management.md)。

## 结果核验

点赞和收藏先核对笔记身份及当前状态，已达到目标则不点击；点击后只在核验状态达到目标时报告成功。评论和回复必须识别新出现的己方评论及对应目标，已有相同文字不构成成功证据。任何提交后未确认的结果都不能自动重发。

通知返回 `liked`、各内容的状态及 `content_available`；`null` 表示缺少证据。`can_reply=false` 表示无评论目标或内容不可用，`null` 不证明有回复入口。保留不可用记录供回溯，整理待处理时不要把它们当作可直接回复。
...
- **评论发送失败**：检查内容是否包含敏感词。
- **点赞/收藏未确认**：`status=unknown` 时先重新读取网页状态，不能直接重复点击；`success=false` 对应非零退出码。
