...
| `list-managed-notes` | 读取创作后台笔记 |
| `get-note-status` | 查询笔记状态 |
| `list-drafts` | 读取浏览器本地草稿 |
| `open-draft` | 预览或显式打开指定草稿编辑 |

---

## 已有笔记与草稿

`list-managed-notes` 读取创作后台笔记，`get-note-status` 查询指定笔记状态；未知状态保留未知。`list-my-notes` 属于主页列表，不替代后台审核状态。

`list-drafts` 读取当前浏览器本地草稿，不能称为全账号云端草稿。`open-draft` 通过 UUID、类型和完整标题定位，默认仅预览；用户明确要继续编辑时加 `--open`，不自动保存或发布。

当前编辑器已有内容时先处理它，不绕过导航保护。详细参数和范围见 [后台笔记与本地草稿管理](../../docs/publish-management.md)。

## 输入判断

用户要求查询笔记状态、列草稿或继续已有草稿时，优先执行上面的管理流程，不进入新建发布流程。
...
| `--schedule-at ISO8601` | 带时区的定时发布时间，未来1小时至14天内，按北京时间填写 |
| `--original` | 声明原创，设置失败时阻止发布 |
| `--visibility` | 可见范围 |

## 结果确认

填写完成不等于发布完成。提交后无可靠成功反馈返回未确认错误（`status=unknown`），先核对创作后台，不自动再次提交；明确失败按错误原因处理。用户要求原创时，声明设置失败不能继续发布。定时参数须带时区，例如 `2026-09-15T10:30:00+08:00`，并在页面操作前校验时间窗口。
